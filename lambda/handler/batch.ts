...
export const handler: Handler = async (event, context) => {
    ...
    if (isAnonymous) {
        ...
        try {
            const secretHash = crypto.createHmac('sha256', clientSecret).update(id + clientId).digest('base64');
    ...
    const objectsProc = requestBody.objects.map(async ({ oid, size }): Promise<BatchRespObject> => {
        ...
        const expiresAt = dayjs().add(env.expires, 's').format('YYYY-MM-DDTHH:mm:ssZ');
